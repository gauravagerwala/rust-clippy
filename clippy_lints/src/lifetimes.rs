...
use clippy_utils::source::{SourceFileRange, SpanExt, SpanLike, gen_list_item_removal_sugg_into};
...
use rustc_hir::{
    AmbigArg, FnPtrTy, FnRetTy, FnSig, GenericParam, GenericParamKind, Generics, Impl, ImplItem, ImplItemId,
    ImplItemKind, Item, ItemId, ItemKind, Lifetime, LifetimeKind, LifetimeParamKind, OwnerId, PatKind, PolyTraitRef,
        ...
        if lt_uses.is_empty() {
            ...
            elidible_sugg.extend(lt_uses.iter().map(|x| {
                ...
                match x.use_site {
                    ...
                    LifetimeUseSite::Ref => {
                        if let Some(sp) = x.span.map_range(cx, SourceFileRange::add_trailing_whitespace) {
                            (sp, String::new())
                        } else {
...
fn report_lifetimes_inner(
    ...
    } else if !gen_list_item_removal_sugg_into(cx, ctxt, ',', lt_spans.iter().map(|sp| sp.into_range()), &mut sugg) {
        return;
    }
...
impl<'tcx> Visitor<'tcx> for LifetimeVisitor<'_, '_, 'tcx> {
    ...
    fn visit_impl_item_ref(&mut self, item: &'tcx ImplItemId) -> Self::Result {
        let item = self.cx.lcx.tcx.hir_impl_item(*item);
